    ...
    return (
      <div className="flex gap-1">
        {cards.map((_, i) => (
          <div
            key={i}
            className="w-14 h-20 bg-gradient-to-br from-blue-900 to-blue-800 border-2 border-blue-600 rounded-lg flex items-center justify-center shadow-lg transform hover:scale-105 transition-transform"
          >
            <div className="w-10 h-12 bg-gradient-to-br from-blue-800 to-blue-700 rounded border border-blue-600 shadow-inner"></div>
          </div>
        ))}
      </div>
    )
  ...
  return (
    <div className="flex gap-1">
      {cards.map((card, i) => (
        <div
          key={i}
          className="w-14 h-20 bg-gradient-to-br from-white to-gray-100 border-2 border-gray-300 rounded-lg flex items-center justify-center text-lg font-bold shadow-lg transform hover:scale-105 transition-transform"
        >
...
function ChipStack({ amount, size = "md" }: { amount: number; size?: "sm" | "md" | "lg" }) {
  const chipCount = Math.min(Math.floor(amount / 10) + 1, 8)
  const sizeClasses = {
    sm: "w-6 h-6",
    md: "w-8 h-8",
    lg: "w-10 h-10",
  ...
  return (
    <div className="relative flex items-end justify-center">
      ...
      <div className="mt-8 text-xs font-bold text-center min-w-16">${amount}</div>
    </div>
  )
}
...
function QValueDisplay({
  ...
  return (
    <Card className="w-72 bg-gradient-to-br from-purple-50 to-blue-50 border-purple-200">
      ...
      <CardContent className="space-y-3">
        {normalizedValues.map(({ action, probability }) => (
          <div key={action} className="flex justify-between items-center">
            <span
              className={`text-sm font-medium ${action === recommendedAction ? "text-green-700 font-bold" : "text-gray-700"}`}
            >
              {action}
            </span>
            <div className="flex items-center gap-2">
              ...
              <span className="text-xs w-12 text-right font-mono">{(probability * 100).toFixed(1)}%</span>
            </div>
          </div>
        ))}
        {recommendedAction && (
...
function PlayerActionIndicator({ action, isVisible }: { action?: string; isVisible: boolean }) {
  if (!isVisible || !action) return null

  return (
    <div className="absolute -top-8 left-1/2 transform -translate-x-1/2 z-10">
      <div className="bg-blue-600 text-white px-3 py-1 rounded-full text-sm font-medium shadow-lg animate-pulse">
        {action}
      </div>
    </div>
  )
...
export default function PokerGame() {
  ...
  const pollGameState = async () => {
    ...
    try {
      ...
      if (data.current_player !== gameState.current_player) {
        const aiPlayer = gameState.players[gameState.current_player]
        if (aiPlayer && !aiPlayer.is_human) {
          setLastAction({ playerId: gameState.current_player, action: "Thinking..." })
  ...
  // Clear action indicator after delay
  useEffect(() => {
    if (lastAction) {
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="relative">
          ...
          <div className="text-center mb-8">
            <h3 className="text-white mb-4 text-lg font-semibold">Community Cards</h3>
          ...
          <div className="relative w-full h-[500px]">
            ...
            <div className="absolute top-0 left-8">
              <div className="relative">
                ...
                />
                <Card className="w-56 bg-gradient-to-br from-blue-50 to-blue-100 border-blue-200 shadow-xl">
                  <CardContent className="p-4">
                    <div className="text-center space-y-3">
                      <h4 className="font-bold text-blue-800">{aiPlayers[0]?.name}</h4>
                      ...
                      {aiPlayers[0]?.folded && <Badge variant="destructive">Folded</Badge>}
                      {gameState.current_player === 1 && <Badge className="bg-blue-600">Current Player</Badge>}
                    </div>
                  </CardContent>
                </Card>
              </div>
            ...
            <div className="absolute top-0 right-8">
              <div className="relative">
                ...
                />
                <Card className="w-56 bg-gradient-to-br from-purple-50 to-purple-100 border-purple-200 shadow-xl">
                  <CardContent className="p-4">
                    <div className="text-center space-y-3">
                      <h4 className="font-bold text-purple-800">{aiPlayers[1]?.name}</h4>
                      ...
                      {aiPlayers[1]?.folded && <Badge variant="destructive">Folded</Badge>}
                      {gameState.current_player === 2 && <Badge className="bg-purple-600">Current Player</Badge>}
                    </div>
                  </CardContent>
                </Card>
              </div>
            ...
            <div className="absolute bottom-0 left-1/2 transform -translate-x-1/2">
              <div className="relative">
                ...
                />
                <Card className="w-72 bg-gradient-to-br from-green-50 to-green-100 border-green-200 shadow-xl">
                  <CardContent className="p-4">
                    <div className="text-center space-y-3">
                      <h4 className="font-bold text-green-800">{humanPlayer?.name} (You)</h4>
                      ...
                      {humanPlayer?.folded && <Badge variant="destructive">Folded</Badge>}
                      {isPlayerTurn && <Badge className="bg-green-600">Your Turn</Badge>}
                    </div>
                  </CardContent>
                </Card>
              </div>
            ...
            {/* Q-Values Display (Right Side) - Show during player's turn */}
            {isPlayerTurn && gameState.q_values && (
          ...
          {isPlayerTurn && !gameState.game_over && (
            <div className="mt-8 text-center">
              <h3 className="text-white mb-4 text-lg font-semibold">Your Turn - Choose Action</h3>
